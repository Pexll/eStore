...
export default function RegisterForm() {
  ...
  const isStepValid = (step: number): boolean => {
    switch (step) {
      ...
      case 1:
        return !!watchedFields.firstName && 
              ...
              <div className="space-y-2">
                {['marketing', 'updates', 'security'].map((type) => (
                  <label key={type} className="flex items-center space-x-2">
                    <input
                      type="checkbox"
                      {...register(`notifications.${type}` as 'notifications.marketing' | 'notifications.updates' | 'notifications.security')}
        ...
        <div className="flex justify-between items-center mb-6">
          {steps.map((step, index) => (
            ...
            >
              ...
              <span className="text-sm mt-3 font-medium text-neutral-800 dark:text-neutral-200">
                {step.title}
              </span>
